...
int execk(uint32_t inode_id,int stdout,char** para,uint32_t para_num){
    ...
    // mnode* head_node_phy=(mnode*)(global_pcb_list[new_pid].satp&0xfffff000);
    // head_node_phy->next=0;
    // head_node_phy->size=0xffffffff;
    // printk("global_pcb_list[%d].satp:%x\n",new_pid,global_pcb_list[new_pid].satp);

    // //以下初始化栈空间
    // uint32_t* stack_bottom=(uint32_t*)mallock(0x00100000,(uint32_t*)(global_pcb_list[new_pid].satp&0xfffff000));//1MB栈空间
    ...
    //     if(prog_head_type==1){
    //         //加载可执行文件
    //         uint8_t* program_start=(uint8_t*)mallock(prog_head_memsize,(uint32_t*)(global_pcb_list[new_pid].satp&0xfffff000));//程序存放位置
    ...
    //     uint8_t* para_addr=(uint8_t*)mallock(para_size,(uint32_t*)(global_pcb_list[new_pid].satp&0xfffff000));
